...
export function createParticipantSchema() {
  return z
    ...
    .refine(
      (data) =>
        data.beneficiaryType !== 'autre' ||
        (data.beneficiaryTypeOther && data.beneficiaryTypeOther.length > 0),
      {
        message: i18n.t('common:validation.beneficiaryTypeOtherRequired'),
        path: ['beneficiaryTypeOther'],
      },
    )
}
...
export function createEventSchema() {
  return z
    .object({
      title: z
        .string()
        .min(1, i18n.t('organizer:validation.titleRequired'))
        .max(200, i18n.t('organizer:validation.titleTooLong')),
